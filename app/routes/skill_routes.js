...
router.get('/skills/:id', requireToken, (req, res, next) => {
  ...
  Skill.findById(req.params.id)
    ...
    .then(skill => res.status(200).json({ skill: skill }))
    // if an error occurs, pass it to the handler
    .catch(next)
})
...
router.patch('/skills/:id', requireToken, removeBlanks, (req, res, next) => {
  ...
  Skill.findById(req.params.id)
    .then(handle404)
    // ensure the signed in user (req.user.id) is the same as the skill's owner (skill.owner)
    .then(skill => requireOwnership(req, skill))
    // updating skill object with skillData
    .then(skill => skill.updateOne(req.body.skill))
...
router.delete('/skills/:id', requireToken, (req, res, next) => {
  Skill.findById(req.params.id)
    ...
    // ensure the signed in user (req.user.id) is the same as the skill's owner (skill.owner)
    .then(skill => requireOwnership(req, skill))
    // delete skill from mongodb
    .then(skill => skill.deleteOne())
